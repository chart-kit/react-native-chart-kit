...
Commands refreshed on May 5, 2026 at commit `a2b855f`. The native release-build dry-run row verifies the command path and generated build commands, not a native build artifact. Browser-backed checks require localhost server access for Playwright's web server.
...
| Public surface audit   | `npm run surface:check`               | Passed: compatibility exports, modern exports, Pro-candidate metadata, and package-boundary audit    |
...
| Skia package boundary   | `packages/skia-renderer`; `npm run skia:typecheck`; `packages/react-native/test/line-renderer.test.ts`                                                              | Preview primitive adapter exists and `LineChart` accepts the injected renderer for its main plot, range selector, sticky-axis, path-local area gradients, threshold rect clips, measured text anchors, default marker, default legend, default tooltip, and debug-layout surfaces; local renderer contract coverage exists; native install and native renderer parity evidence are still pending H4 |
| Pro package boundary    | `packages/pro`; `npm run pro:typecheck`                                                                                                                             | Preview feature registry, surface-boundary metadata, and injected React Native composition helper now mirror the H4 commercial buckets: layout engine, interactions, chart types, export, templates, performance, Skia, and accessibility reports. License gating and paid implementations still pending H4                                                                                         |
...
| Public export surface   | `scripts/verify-public-surface.mjs`; `scripts/verify-package-boundaries.mjs`; `npm run surface:check`                                                               | Covered for root compatibility exports, modern v2 exports, Pro-candidate metadata, and package-boundary constraints                                                                                                                                                                                                                                                                                 |
