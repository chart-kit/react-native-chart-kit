...
| Interaction                     | selection provider, tap/scrub, tooltips, crosshair, bar/pie/donut/candlestick interactions, `docs/release/native-runtime-qa.md`                                                                                                     | Covered on web showcase; native runtime protocol exists; completed device evidence still missing                |
...
| Benchmark                       | `npm run benchmark`, `docs/release/h5-beta-gate-evidence.md`, `docs/release/native-performance-benchmark.md`                                                                                                                        | Covered for core geometry and web scrub timing; native performance protocol exists; release-device data missing |
...
| Release packet                  | beta checklist, H5 evidence, owner decision memo, known issues, changelog, `scripts/check-release-gates.mjs`                                                                                                                        | Covered for owner review and executable gate reporting                                                          |
...
| Native e2e/runtime QA        | Protocol exists, but no completed iOS/Android device runtime log                                                | Gestures, scroll conflicts, text rendering, and release behavior can differ on device         |
...
| H4 Pro scope                 | Draft and preview `packages/pro` boundary exist, but no owner-approved free/Pro split or license implementation | Required before labeling animation/range/zoom/financial/Skia features as free or Pro          |
| Skia renderer                | `packages/skia-renderer` exposes an injected primitive adapter, but chart components do not consume it          | Chart integration, native install evidence, and parity tests are not implemented              |
| Native performance benchmark | Protocol exists, but no release-device timing/FPS/memory log                                                    | Current benchmark is core geometry plus web scrub timing                                      |
