...
export default function App() {
  ...
  const takeoverStory = useMemo(
    () =>
      pageStoryGroups
        ...
        .find((story) => story.presentation === "takeover"),
    [pageStoryGroups]
  );
  ...
  if (takeoverStory) {
    ...
    const isHealthTakeover = takeoverStory.id === "landing-health-activity-app";
    ...
    return (
      <GestureHandlerRootView style={styles.gestureRoot}>
        <View
          style={[
            styles.takeoverRoot,
            isHealthTakeover && { backgroundColor: "#f7f8fc" }
          ]}
        >
          <StatusBar
            barStyle={isHealthTakeover ? "dark-content" : "light-content"}
          />
          <ChartKitProvider
            mode={themeMode}
            preset={chartPreset}
